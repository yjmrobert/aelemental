...
public class ParsedFile
{
    ...
    {
        Pathname = path;
        if (!string.IsNullOrWhiteSpace(additionalCS)) AdditionalCode = ReadLines(additionalCS)?.ToList();
    ...
    public static string[] ReadLines(string pathname)
    {
        if (!File.Exists(pathname))
            return new[]
            {
                $"Title: N/A (File {pathname} not found)", $"Description: N/A (File {pathname} not found)",
    ...
    public static string[] ReadScssLines(string pathname)
    {
        ...
        if (!File.Exists(scssPathname)) return null;
