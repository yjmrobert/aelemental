...
public class ModelFormTests
{
    ...
    public void TestTransportStarshipCategories()
    {
        ...
        Assert.Collection(s1, e => Assert.Null(e.category), e => Assert.Equal("Identification", e.category),
            e => Assert.Equal("Details", e.category));
        //var (c1, l1) = s1[0];
    }
    ...
    public void TestTransportStarshipCategoriesVisibility()
    {
        ...
        var allCats = typeof(TransportStarship).GetAeModelFormCategories().Select(elem => (elem.category,
                visibleProperties: elem.properties.Select(l => l.Where(l => model.IsVisible(l)).ToList()).ToList()))
            .ToList();
    ...
    public void TestPropertyName()
    {
        Assert.Equal("Classification",
            AeModelFormTools.WithPropertyExpression<InteractiveStarship>(p => p.Classification).Name);
    }
}